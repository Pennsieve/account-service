...
func TestAccountStore_Insert(t *testing.T) {
	tableName := "accounts_test_insert"
	...
	defer func() {
		_ = DeleteTable(dynamoDBClient, tableName)
	}()

	// Wait for table to be active
	time.Sleep(100 * time.Millisecond)

	store := store_dynamodb.NewAccountDatabaseStore(dynamoDBClient, tableName)
...
func TestAccountStore_GetById_NotFound(t *testing.T) {
	tableName := "accounts_test_getbyid_notfound"
	dynamoDBClient := getClient()
...
func TestAccountStore_Get_FilterByOrganization(t *testing.T) {
	tableName := "accounts_test_get_filter"
	dynamoDBClient := getClient()
...
func TestAccountStore_Update(t *testing.T) {
	tableName := "accounts_test_update"
	dynamoDBClient := getClient()
...
func TestAccountStore_Update_NonExistent(t *testing.T) {
	tableName := "accounts_test_update_nonexistent"
	dynamoDBClient := getClient()
	...
	// The update might succeed (upsert behavior) or fail depending on implementation
	// Check the actual behavior and adjust test accordingly
	t.Logf("Update non-existent account result: %v", err)
}
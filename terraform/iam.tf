...
# Bucket configuration reads, for consumers that report on a storage bucket
# without needing a byte of what is in it (the infra dashboard's Fleet screen
# is the first). Deliberately separate from the read policy: reporting on a
# bucket must not imply access to research data. Bucket-level ARNs only —
# these operations take no object ARN. Regenerated by the same Lambda as the
# read and write policies whenever storage nodes change.
resource "aws_iam_policy" "storage_metadata" {
  name = "${var.environment_name}-storage-bucket-metadata"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Sid      = "StorageBucketMetadata"
      Effect   = "Allow"
      Action   = ["s3:GetBucketLocation", "s3:GetEncryptionConfiguration", "s3:GetBucketVersioning", "s3:GetBucketPublicAccessBlock", "s3:GetBucketPolicyStatus", "s3:GetBucketTagging"]
      Resource = ["arn:aws:s3:::placeholder"]
    }]
  })
